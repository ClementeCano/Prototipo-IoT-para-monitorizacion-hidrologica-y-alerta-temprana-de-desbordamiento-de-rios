...
def fetch_aemet_municipio_horaria(municipio: str = "09219") -> List[Dict[str, Any]]:
    api_key = os.getenv("AEMET_APIKEY", "")
    if not api_key:
        raise RuntimeError("Falta AEMET_APIKEY (variable de entorno)")
...
def _parse_periodo_to_interval(fecha_iso: str, periodo: str) -> Optional[Tuple[datetime, datetime]]:
    ...
    """
    try:
        base_date = datetime.fromisoformat(fecha_iso).replace(tzinfo=TZ, hour=0, minute=0, second=0, microsecond=0)
    except Exception:
        return None
...
def extract_rain_forecast_mm(aemet_data: List[Dict[str, Any]], hours_ahead: int = 24, list_hours: int = 12) -> Dict[str, Any]:
    """
    Predicción de cantidad (mm):
      - sumas 6h/24h
      - máximos horarios 6h/24h
      - lista próxima N horas: [{hora, mm}]
    """
    now = datetime.now(TZ)
    ...
    if not aemet_data:
        return {
            "aemet_mm_6h_sum": 0.0, "aemet_mm_24h_sum": 0.0,
            "aemet_mm_6h_max": 0.0, "aemet_mm_24h_max": 0.0,
            "aemet_mm_next_hours": []
        }

    item = aemet_data[0] if isinstance(aemet_data[0], dict) else None
    if not item:
        return {
            "aemet_mm_6h_sum": 0.0, "aemet_mm_24h_sum": 0.0,
            "aemet_mm_6h_max": 0.0, "aemet_mm_24h_max": 0.0,
...
def extract_prob_precip_summary(aemet_data: List[Dict[str, Any]], hours_ahead: int = 24) -> Dict[str, Optional[int]]:
    ...
    Probabilidad (%) de precipitación:
      - máximo en próximas 6h
      - máximo en próximas 24h
    Usa probPrecipitacion (periodos tipo 1319, 1901, 00-06, etc.).
    """
    now = datetime.now(TZ)
    limit_6h = now + timedelta(hours=6)
    limit_24h = now + timedelta(hours=hours_ahead)
    ...
    for d in dias:
        ...
        for p in pp:
            ...
            try:
                prob = int(val) if val is not None and str(val).strip() != "" else None
            except:
                prob = None
            if prob is None:
                continue

            interval = _parse_periodo_to_interval(fecha, str(periodo))
            if not interval:
                continue
            start, end = interval

            # overlap ventanas futuras
